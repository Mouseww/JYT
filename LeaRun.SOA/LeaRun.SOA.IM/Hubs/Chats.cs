    ...
    public class Chats : Hub
    {
        ...
        private void UpdateUserListState(string userId, int isOnLine)
        {
            if (UserStorage.userAllList.ContainsKey(userId))
            {
                ...
                UserStorage.userAllList[userId].F_UserOnLine = isOnLine;
            }
        }
        /// <summary>
        /// 更新用户组连接Id信息
        ...
        public void imSendLastUser()
        {
            ...
            {
                item.F_OtherId = item.F_SendId == userId ? item.F_UserId : item.F_SendId;

                if (item.F_OtherId != null && !dic.ContainsKey(item.F_OtherId))
                {
                    IEnumerable<IMReadModel> imReadModel = imcontentbll.GetListOneToOne(pagination, userId, item.F_OtherId);
                    foreach (var i in imReadModel)
                    {
                        item.F_IMContent = i.F_Content;
                        item.F_IMCreateTime = i.F_CreateDate;
                    }
                    dic.Add(item.F_OtherId, item);
                }
            }
        ...
        public int createGroup(string groupName, List<string> userIdList)
        {
            ...
            {
                string userId = GetUserId();
                string userName = UserStorage.userAllList[userId].F_RealName;
        ...
        public int addGroupUserId(string groupId, string userId)
        {
            try
            {
                ...
                string createUserName = UserStorage.userAllList[userId].F_RealName;
        ...
        public bool upDateUserList(IMUserModel userlist, bool isAdd)
        {
            if (isAdd)//增加用户信息
            {
                if (UserStorage.userAllList.ContainsKey(userlist.F_UserId))
                {
                    IMUserModel one = UserStorage.userAllList[userlist.F_UserId];
                    UserStorage.userAllList.Remove(userlist.F_UserId);
                    userlist.F_UserOnLine = one.F_UserOnLine;
                    UserStorage.userAllList.Add(userlist.F_UserId, userlist);
                }
                else
                {
                    UserStorage.userAllList.Add(userlist.F_UserId, userlist);
