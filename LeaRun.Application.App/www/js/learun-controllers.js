...
.controller('TestCtrl', ['$scope', '$learunHttp', function () {
    $scope.data = "测试";
  ...
  function ($scope,$timeout, $ionicModal, $learunPopup,$ionicLoading, $learunTopAlert, $learunTriggerRefresh,$learunSelectModal,$learunDataIsAll, lrmBusinesss,lrmBaseInfo) {
    ...
    $scope.openSourceSelectModal = function () {//打开选择商机来源
      $learunSelectModal({
        title:"商机来源",
        bgAllColor:"royal-bg",
        selectValue:$scope.editData.f_SourceId,
        text:"f_ItemName",
        value:"f_ItemValue",
        data:lrmBusinesss.getChanceSource(),
        onChange: function (item) {
          $scope.editData.f_SourceId = item.value;
          $scope.editData.f_SourceName = item.text;
        }
      });
    };
    $scope.openStageSelectModal = function () {//打开选择商机阶段
      $learunSelectModal({
        title:"商机阶段",
        selectValue:$scope.editData.f_StageId,
        text:"f_ItemName",
        value:"f_ItemValue",
        bgColor:"bgColor",
        data:lrmBusinesss.getChancePhases(),
        onChange: function (item) {
          $scope.editData.f_StageId = item.value;
          $scope.editData.f_StageName = item.text;
          $scope.editData.f_BgStageColor = item.bgColor;
        }
      });
    };
    $scope.openTraceUserSelectModal = function(){
      $learunSelectModal({
        title:"跟进人员",
        bgAllColor:"calm-bg",
        selectValue:$scope.editData.f_TraceUserId,
        text:"f_RealName",
        value:"f_UserId",
        data:lrmBaseInfo.getUserInfoList("2f077ff9-5a6b-46b3-ae60-c5acdc9a48f1"),
        onChange: function (item) {
          $scope.editData.f_TraceUserId = item.value;
          $scope.editData.f_TraceUserName = item.text;
        }
      });
    };
