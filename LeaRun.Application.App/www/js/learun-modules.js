...
//主页应用列表
  .factory('HomeApps', function () {
    ...
    , {
        ...
        color: "bgcolor_d",
        viewid: 'home-receivable.html',
    }, {
        name: "收款报表",
        icon: "ion-ios-pulse",
        color: "bgcolor_e",
        viewid: 'home-receivableReport.html',
    }, {
        name: "协同办公",
    ...
    function translateData(data, obj) {
      //console.log(data);
        for (var i in data) {
          ...
          if(item.f_CustIndustryId == null)
          {
            data[i].f_CustIndustryName = "未知业";
            data[i].f_CustIndustrybgColor = "dark-bg";
          }
          else{
            data[i].f_CustIndustryName = custTrade[item.f_CustIndustryId].f_ItemName;
            data[i].f_CustIndustrybgColor = custTrade[item.f_CustIndustryId].bgColor;
          }

          data[i].f_CustLevelName = custLevel[item.f_CustLevelId].f_ItemName;
          data[i].f_CustTypeName = custType[item.f_CustTypeId].f_ItemName;
          data[i].f_CustDegreeName = custDegree[item.f_CustDegreeId].f_ItemName;

          if (data[i].f_Province != null)
          {
              data[i].f_ProvinceName = AreaInfo.getProvinceName(item.f_Province);
              if (data[i].f_City != null) {
                  data[i].f_CityName = AreaInfo.getCityName(item.f_Province, item.f_City);
              }
          }
          data[i].f_LastDate = item.f_ModifyDate == null ? $learunFormatDate(item.f_CreateDate, 'MM-dd') : $learunFormatDate(item.f_ModifyDate, 'MM-dd');
          data[i].f_CreateDate = $learunFormatDate(item.f_CreateDate, 'yyyy-MM-dd hh:mm');
          data[i].f_ModifyDate = $learunFormatDate(item.f_ModifyDate, 'yyyy-MM-dd hh:mm');
          obj.push(data[i]);
        }
    };
    //获取数据
    ...
    return {
      ...
      get: function (custId) {
        for (var i = 0; i < custList.customers.length; i++) {
          if (custList.customers[i].f_CustomerId === custId) {
        ...
        if(custListSearch.customers != undefined)
        {
          for (var i = 0; i < custListSearch.customers.length; i++) {
            if (custListSearch.customers[i].f_CustomerId === custId) {
  ...
  .factory('lrmBusinesss',['$learunFormatDate','$learunGetDataItem','$learunHttp','ApiUrl','$learunTopAlert',
    ...
    function translateData(data,obj) {
      //console.log(data);
        for (var i in data) {
          var item = data[i];
          data[i].f_StageName =  chancePhases[data[i].f_StageId].f_ItemName;
          data[i].f_BgStageColor = chancePhases[data[i].f_StageId].bgColor;
          data[i].f_SourceName = chanceSource[data[i].f_SourceId].f_ItemName;
          data[i].f_LastDate =  $learunFormatDate(item.f_ModifyDate, 'MM-dd');
          data[i].f_CreateDate = $learunFormatDate(item.f_CreateDate, 'yyyy-MM-dd hh:mm');
          data[i].f_ModifyDate = $learunFormatDate(item.f_ModifyDate, 'yyyy-MM-dd hh:mm');
          data[i].f_DealDate = $learunFormatDate(item.DealDate, 'yyyy-MM-dd');
          obj.push(data[i]);
        }
    };
    //获取数据方法
    ...
    return {
      ...
      get: function (chanceId) {
        for (var i = 0; i < businessList.businesss.length; i++) {
          if (businessList.businesss[i].f_ChanceId === chanceId) {
        ...
        if(businessListSearch.businesss != undefined){
          for (var i = 0; i < businessListSearch.businesss.length; i++) {
            if (businessListSearch.businesss[i].f_ChanceId === chanceId) {
    ...
    .factory('lrmIM', ['$q', 'UserInfo', 'SignalrUrl', '$learunLoadJs', '$learunFormatDate', function ($q, UserInfo, SignalrUrl, $learunLoadJs, $learunFormatDate) {
        ...
        function LastUserListToArray()
        {
            ...
            for (var i in imServer.lastUserList) {
                imServer.lastUserList[i].F_IMCreateTime = $learunFormatDate(imServer.lastUserList[i].F_IMCreateTime, "MM-dd hh:mm");
                        ...
                        function (data) {
                            if (data == "success") {
                                ...
                                chat.client.IMUpdateUserList = function (userAllList, onLineUserList) {
                                    ...
                                    for (var Id in userAllList){
                                        ...
                                        if (Id != imServer.userInfo.userid) {
                                            if (imServer.userAllListByDepartment[item.F_DepartmentId] == undefined) {
                                                imServer.userAllListByDepartment[item.F_DepartmentId] = {};
                                                imServer.userAllListByDepartment[item.F_DepartmentId].DepartmentName = item.F_DepartmentId;
                                                if (item.UserOnLine == 1) {
                                                    imServer.userAllListByDepartment[item.F_DepartmentId].onLineNum = 1;
                                                }
                                                else {
                                                    imServer.userAllListByDepartment[item.F_DepartmentId].onLineNum = 0;
                                                }
                                                imServer.userAllListByDepartment[item.F_DepartmentId].UserList = [];
                                                imServer.userAllListByDepartment[item.F_DepartmentId].UserList.push(item);
                                            }
                                            else {
                                                if (item.UserOnLine == 1) {
                                                    imServer.userAllListByDepartment[item.F_DepartmentId].onLineNum++;
                                                }
                                                imServer.userAllListByDepartment[item.F_DepartmentId].UserList.push(item);
                                            }
                                        }
                                ...
                                chat.client.IMUpdateUserStatus = function (userId, isOnLine) {
                                    imServer.userAllList[userId].F_UserOnLine = isOnLine;
                                    if ($scope != null && $scope != undefined) {
                                        $scope.$apply();
                                    }
                                }
                                ...
                                chat.client.RevMessage = function (formUser, message, dateTime) {
                                    if (imServer.currentUserId != "") {
                                        ...
                                        if (imServer.lastUserList[imServer.currentUserId] == undefined) {
                                            ...
                                            imServer.lastUserList[imServer.currentUserId].F_UnReadNum = 0;
                                            imServer.lastUserList[imServer.currentUserId].F_OtherId = imServer.currentUserId;
                                            imServer.lastUserList[imServer.currentUserId].F_SendId = formUser;
                                            var id = (formUser == imServer.userInfo.userid ? imServer.currentUserId : formUser);
                                            imServer.lastUserList[imServer.currentUserId].F_UserId = id;
                                        }
                                        imServer.lastUserList[imServer.currentUserId].F_IMCreateTime = dateTime;
                                        imServer.lastUserList[imServer.currentUserId].F_IMContent = message;

                                    } else {
                                        if (imServer.lastUserList[formUser] == undefined)
                                        {
                                            imServer.lastUserList[formUser] = {};
                                            imServer.lastUserList[formUser].F_UnReadNum = 0;
                                        }
                                        imServer.lastUserList[formUser].F_IMContent = message;
                                        imServer.lastUserList[formUser].F_OtherId = formUser;
                                        imServer.lastUserList[formUser].F_SendId = formUser;

                                        imServer.lastUserList[formUser].F_IMCreateTime = dateTime;
                                        imServer.lastUserList[formUser].F_UserId = imServer.userInfo.userid;

                                        imServer.lastUserList[formUser].F_UnReadNum++;
                                    }
    ...
    function ($learunHttp, ApiUrl,$learunTopAlert,$learunFormatDate) {
      ...
      function getData(page,queryData, obj,callback) {
        $learunHttp.post({
          ...
          "data": { "page": page, "rows": 6, "sidx": "F_ModifyDate", "sord": "desc", "queryData": JSON.stringify(queryData) },
          "isverify": true,
          "success": function (data) {
            if(page == 1) {
              obj.orderInfos =[];
      ...
      return{
        ...
        get: function (orderId) {
          //console.log(orderId);
          //console.log(orderInfoList.orderInfos.length);
          //console.log(JSON.stringify(orderInfoList.orderInfos));
          for (var i = 0; i < orderInfoList.orderInfos.length; i++) {
            console.log(orderInfoList.orderInfos[i].f_OrderId);
            if (orderInfoList.orderInfos[i].f_OrderId == orderId) {
      ...
      return{
        ...
        get: function (receivableId) {
          for (var i = 0; i < receivableList.receivables.length; i++) {
            ...
            if (receivableList.receivables[i].f_OrderId == receivableId) {
