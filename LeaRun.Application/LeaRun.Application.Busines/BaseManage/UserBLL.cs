    ...
    public class UserBLL
    {
        ...
        public UserEntity CheckLogin(string username, string password)
        {
            ...
            if (userEntity != null)
            {
                ...
                {
                    string dbPassword = Md5Helper.MD5(DESEncrypt.Encrypt(password.ToLower(), userEntity.F_Secretkey).ToLower(), 32).ToLower();
                    if (dbPassword == userEntity.F_Password)
                    {
                        DateTime LastVisit = DateTime.Now;
                        int LogOnCount = (userEntity.F_LogOnCount).ToInt() + 1;
                        if (userEntity.F_LastVisit != null)
                        {
                            userEntity.F_PreviousVisit = userEntity.F_LastVisit.ToDate();
                        }
                        userEntity.F_LastVisit = LastVisit;
                        userEntity.F_LogOnCount = LogOnCount;
                        userEntity.F_UserOnLine = 1;
        ...
        public void GetExportList()
        {
            ...
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_account", ExcelColumn = "账户" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_realname", ExcelColumn = "姓名" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_gender", ExcelColumn = "性别" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_birthday", ExcelColumn = "生日" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_mobile", ExcelColumn = "手机", Background = Color.Red });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_telephone", ExcelColumn = "电话", Background = Color.Red });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_wechat", ExcelColumn = "微信" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_manager", ExcelColumn = "主管" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_organize", ExcelColumn = "公司" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_department", ExcelColumn = "部门" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_description", ExcelColumn = "说明" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_createdate", ExcelColumn = "创建日期" });
            excelconfig.ColumnEntity.Add(new ColumnEntity() { Column = "f_createusername", ExcelColumn = "创建人" });
            //调用导出方法
