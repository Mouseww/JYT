        ...
        public static IEnumerable<T> ToAuthorizeData<T>(this IEnumerable<T> data)
        {
            if (data != null)
            {
                if (OperatorProvider.Provider.Current().IsSystem)
                    return data;
                string dataAutor = OperatorProvider.Provider.CurrentDataAuthorize().ReadAutorizeUserId;
                var parameter = Expression.Parameter(typeof(T), "t");
                var authorConditon = Expression.Constant(dataAutor).Call("Contains", parameter.Property("F_CreateUserId"));
