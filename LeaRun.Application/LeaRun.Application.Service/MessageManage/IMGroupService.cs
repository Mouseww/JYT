...
namespace LeaRun.Application.Service.MessageManage
{
    ...
    public class IMGroupService : RepositoryFactory, IMsgGroupService
    {
        ...
        public IEnumerable<IMGroupModel> GetList(string userId)
        {
            ...
            strSql.Append(@"SELECT  g.F_GroupId ,
                                    g.F_FullName AS F_GroupName ,
                                    u.F_UserId ,
                                    u.F_UserGroupId
                            FROM    IM_UserGroup u
                                    LEFT JOIN IM_Group g ON u.F_GroupId = g.F_GroupId
                            WHERE   1 = 1");
            var parameter = new List<DbParameter>();
            //公司主键
            if (!userId.IsEmpty())
            {
                strSql.Append(" AND u.F_UserId = @UserId");
        ...
        {
            var strSql = new StringBuilder();
            strSql.Append(@"SELECT  t.F_GroupId ,
                                    t.F_UserId
                            FROM    IM_UserGroup t
                            WHERE   1 = 1");
            var parameter = new List<DbParameter>();
            ...
            if (!groupId.IsEmpty())
            {
                strSql.Append(" AND u.F_GroupId = @GroupId");
            ...
            else
            {
                ...
                try
                {
                    ...
                    foreach (string userOne in userIdList)
                    {
                        ...
                        msgusergroupentity.F_GroupId = entity.F_GroupId;
                        msgusergroupentity.F_UserId = userOne;
                        msgusergroupentity.F_CreateUserId = entity.F_CreateUserId;
                        msgusergroupentity.F_CreateUserName = entity.F_CreateUserName;
