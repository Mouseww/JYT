...
namespace LeaRun.Application.Service.MessageManage
{
    ...
    public class IMContentService : RepositoryFactory, IMsgContentService
    {
        ...
        public IEnumerable<IMReadModel> GetList(Pagination pagination, string userId, string sendId,string flag = "1")
        {
            ...
            strSql.Append(@"SELECT  * from(
                            SELECT  t1.F_ReadId ,
                                    ...
                                    t1.F_UserId ,
                                    t2.F_SendId ,
                                    t2.F_MsgContent as F_Content
                            FROM    IM_Read t1
                                    INNER JOIN IM_Content t2 ON t1.F_ContentId = t2.F_ContentId and t2.F_IsGroup = 0)T 
                            WHERE   1=1 ");
            ...
            if (!userId.IsEmpty())
            {
                ...
                {
                    strSql.Append(" AND ( T.F_UserId = @UserId or T.F_UserId = @UserId2 )");
                ...
                {
                    strSql.Append(" AND T.F_UserId = @UserId");
                    parameter.Add(DbParameters.CreateDbParameter("@UserId", userId.ToString()));
                }
            }
            //信息发送者
            if (!sendId.IsEmpty())
            {
                ...
                {
                    strSql.Append(" AND ( T.F_SendId = @SendId or T.F_SendId = @SendId2 )");
                ...
                {
                    strSql.Append(" AND T.F_SendId = @SendId");
                    parameter.Add(DbParameters.CreateDbParameter("@SendId", sendId.ToString()));
                }
            }
        ...
        {
            var strSql = new StringBuilder();
            strSql.Append(@"SELECT  t.F_ContentId ,
                                    t.F_SendId ,
                                    t.F_ToId ,
                                    t.F_MsgContent ,
                                    t.F_CreateDate
            ...
            {
                strSql.Append(" AND t.F_ToId = @ToId");
        ...
        public IEnumerable<IMReadNumModel> GetReadList(string userId)
        {
            ...
            strSql.Append(@"SELECT
	                            SUM( CASE WHEN t1.F_ReadStatus = 0 AND t1.F_SendId != @UserId THEN 1 ELSE 0 END )AS F_UnReadNum,
	                            t1.F_SendId,
	                            t1.F_UserId
                            FROM
	                            IM_Read t1
            ...
            if (!userId.IsEmpty())
            {
                strSql.Append(" AND ( t1.F_UserId = @UserId OR t1.F_SendId = @UserId )");
                parameter.Add(DbParameters.CreateDbParameter("@UserId", userId.ToString()));
            }
            strSql.Append(@" GROUP BY t1.F_SendId, t1.F_UserId
                             ORDER BY MAX (t1.F_CreateDate) DESC");
        ...
        {
            var strSql = new StringBuilder();
            strSql.Append(@"SELECT  COUNT(F_ReadId) AS F_UnReadNum
                            FROM    IM_Read 
                            WHERE   1 = 1 ");
            var parameter = new List<DbParameter>();
            ...
            if (!status.IsEmpty())
            {
                strSql.Append(" AND F_ReadStatus = @ReadStatus");
        ...
        public void Add(IMContentEntity entity, DataTable dtGroupUserId)
        {
            ...
            try
            {
                ...
                if (entity.F_IsGroup == 1)
                {
                    foreach (DataRow item in dtGroupUserId.Rows)
                    {
                        IMReadEntity msgreadentity = new IMReadEntity();
                        msgreadentity.Create();
                        msgreadentity.F_ContentId = entity.F_ContentId;
                        msgreadentity.F_UserId = item["userId"].ToString();
                        msgreadentity.F_SendId = entity.F_ToId;//群组消息发送者为群组Id
                        msgreadentity.F_CreateUserId = entity.F_CreateUserId;
                        msgreadentity.F_CreateUserName = entity.F_CreateUserName;
                        msgreadentity.F_ReadStatus = 0;
                ...
                else
                {
                    ...
                    msgreadentity.Create();
                    msgreadentity.F_ContentId = entity.F_ContentId;
                    msgreadentity.F_UserId = entity.F_ToId;
                    msgreadentity.F_SendId = entity.F_SendId;
                    msgreadentity.F_CreateUserId = entity.F_CreateUserId;
                    msgreadentity.F_CreateUserName = entity.F_CreateUserName;
                    msgreadentity.F_ReadStatus = 0;
                    db.Insert<IMReadEntity>(msgreadentity);
        ...
        public int Update(string userId, string sendId, string status)
        {
            ...
            if (!status.IsEmpty())
            {
                strSql.Append(" SET F_ReadStatus = @ReadStatus");
                parameter.Add(DbParameters.CreateDbParameter("@ReadStatus", status));
            }
            ...
            if (!sendId.IsEmpty())
            {
                strSql.Append(" AND F_SendId = @SendId");
