...
namespace LeaRun.Application.Service.SystemManage
{
    ...
    public class AreaService : RepositoryFactory<AreaEntity>, IAreaService
    {
        ...
        {
            return this.BaseRepository().IQueryable(t => t.F_Layer != 4).OrderBy(t => t.F_CreateDate).ToList();
        }
        /// <summary>
        /// 区域列表
        /// </summary>
        ...
        public IEnumerable<AreaEntity> GetList(string parentId, string keyword)
        {
            ...
            {
                expression = expression.And(t => t.F_AreaCode.Contains(keyword));
                expression = expression.Or(t => t.F_AreaName.Contains(keyword));
            }
        ...
        {
            areaEntity.F_SimpleSpelling = SpellHelper.SpellString(areaEntity.F_AreaName).ToLower();
