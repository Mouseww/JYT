...
namespace LeaRun.Application.Service.BaseManage
{
    ...
    public class JobService : RepositoryFactory<RoleEntity>, IJobService
    {
        ...
        public IEnumerable<RoleEntity> GetList()
        {
            ...
            expression = expression.And(t => t.F_Category == 3).And(t => t.F_EnabledMark == 1).And(t => t.F_DeleteMark == 0);
        ...
        public IEnumerable<RoleEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            expression = expression.And(t => t.F_Category == 3);
        ...
        {
            var expression = LinqExtensions.True<RoleEntity>();
            expression = expression.And(t => t.F_EnCode == enCode).And(t => t.F_Category == 3);
        ...
        {
            var expression = LinqExtensions.True<RoleEntity>();
            expression = expression.And(t => t.F_FullName == fullName).And(t => t.F_Category == 3);
            if (!string.IsNullOrEmpty(keyValue))
            {
                expression = expression.And(t => t.F_RoleId != keyValue);
            }
