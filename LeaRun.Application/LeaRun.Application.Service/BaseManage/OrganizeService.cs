...
namespace LeaRun.Application.Service.BaseManage
{
    ...
    public class OrganizeService : RepositoryFactory<OrganizeEntity>, IOrganizeService
    {
        ...
        {
            return this.BaseRepository().IQueryable().OrderByDescending(t => t.F_SortCode).ToList();
        }
        /// <summary>
        /// 机构实体
        /// </summary>
        ...
        public bool ExistFullName(string fullName, string keyValue)
        {
            ...
            {
                expression = expression.And(t => t.F_OrganizeId != keyValue);
            }
        ...
        {
            var expression = LinqExtensions.True<OrganizeEntity>();
            expression = expression.And(t => t.F_ShortName == shortName);
            if (!string.IsNullOrEmpty(keyValue))
            {
                expression = expression.And(t => t.F_OrganizeId != keyValue);
            }
        ...
        {
            int count = this.BaseRepository().IQueryable(t => t.F_ParentId == keyValue).Count();
            if (count > 0)
            {
