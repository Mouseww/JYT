...
namespace LeaRun.Application.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataItemDetailController : MvcControllerBase
    {
        ...
        /// <param name="condition">关键字字段</param>
        ...
        public ActionResult GetTreeListJson(string itemId, string condition, string keyword)
        {
            ...
            if (!string.IsNullOrEmpty(keyword))
            {
                ...
                {
                    case "ItemName":        //项目名
                        data = data.TreeWhere(t => t.F_ItemName.Contains(keyword), "F_ItemDetailId", "F_ParentId");
                        break;
                    case "ItemValue":      //项目值
                        data = data.TreeWhere(t => t.F_ItemValue.Contains(keyword), "F_ItemDetailId", "F_ParentId");
                        break;
                    case "SimpleSpelling": //拼音
                        data = data.TreeWhere(t => t.F_SimpleSpelling.Contains(keyword), "F_ItemDetailId", "F_ParentId");
                        break;
                    default:
                        break;
                }
        ...
        public ActionResult GetDataItemTreeJson(string EnCode)
        {
            ...
            foreach (DataItemModel item in data)
            {
                ...
                bool hasChildren = data.Count(t => t.F_ParentId == item.F_ItemDetailId) == 0 ? false : true;
                tree.id = item.F_ItemDetailId;
                tree.text = item.F_ItemName;
                tree.value = item.F_ItemValue;
                tree.parentId = item.F_ParentId;
                tree.isexpand = true;
                tree.complete = true;
                tree.hasChildren = hasChildren;
