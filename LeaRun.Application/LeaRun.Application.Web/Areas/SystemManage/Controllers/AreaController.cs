    ...
    public class AreaController : MvcControllerBase
    {
        ...
        public ActionResult GetTreeJson(string value)
        {
            ...
            if (filterdata.Count > 0)
            {
                ...
                {
                    bool hasChildren = areaBLL.GetList(item.F_AreaId).ToList().Count == 0 ? false : true;
                    sb.Append("{");
                    sb.Append("\"id\":\"" + item.F_AreaId + "\",");
                    sb.Append("\"text\":\"" + item.F_AreaName + "\",");
                    sb.Append("\"value\":\"" + item.F_AreaId + "\",");
