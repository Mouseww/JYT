...
namespace LeaRun.Application.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataItemController : MvcControllerBase
    {
        ...
        public ActionResult GetTreeJson(string keyword)
        {
            ...
            {
                data = data.TreeWhere(t => t.F_ItemName.Contains(keyword), "F_ITEMID");
            }
            var treeList = new List<TreeEntity>();
            foreach (DataItemEntity item in data)
            {
                TreeEntity tree = new TreeEntity();
                bool hasChildren = data.Count(t => t.F_ParentId == item.F_ItemId) == 0 ? false : true;
                tree.id = item.F_ItemId;
                tree.text = item.F_ItemName;
                tree.value = item.F_ItemCode;
                tree.parentId = item.F_ParentId;
                tree.isexpand = true;
                tree.complete = true;
                tree.Attribute = "isTree";
                tree.AttributeValue = item.F_IsTree.ToString();
