...
namespace LeaRun.Application.Web.Areas.BaseManage.Controllers
{
    ...
    public class DepartmentController : MvcControllerBase
    {
        ...
        public ActionResult GetTreeJson(string organizeId, string keyword)
        {
            ...
            {
                data = data.TreeWhere(t => t.F_FullName.Contains(keyword), "DepartmentId");
            }
        ...
        public ActionResult GetTreeListJson(string condition, string keyword)
        {
            ...
            if (!string.IsNullOrEmpty(condition) && !string.IsNullOrEmpty(keyword))
            {
                ...
                {
                    case "FullName":    //部门名称
                        departmentdata = departmentdata.TreeWhere(t => t.F_FullName.Contains(keyword), "F_DepartmentId");
                        break;
                    case "EnCode":      //部门编号
                        departmentdata = departmentdata.TreeWhere(t => t.F_EnCode.Contains(keyword), "F_DepartmentId");
                        break;
                    case "ShortName":   //部门简称
                        departmentdata = departmentdata.TreeWhere(t => t.F_ShortName.Contains(keyword), "F_DepartmentId");
                        break;
                    case "Manager":     //负责人
                        departmentdata = departmentdata.TreeWhere(t => t.F_Manager.Contains(keyword), "F_DepartmentId");
                        break;
                    case "OuterPhone":  //电话号
                        departmentdata = departmentdata.TreeWhere(t => t.F_OuterPhone.Contains(keyword), "F_DepartmentId");
                        break;
                    case "InnerPhone":  //分机号
                        departmentdata = departmentdata.TreeWhere(t => t.F_Manager.Contains(keyword), "F_DepartmentId");
                        break;
            ...
            foreach (OrganizeEntity item in organizedata)
            {
                ...
                tree.parentId = item.F_ParentId;
                tree.expanded = true;
                item.F_EnCode = ""; item.F_ShortName = ""; item.F_Nature = ""; item.F_Manager = ""; item.F_OuterPhone = ""; item.F_InnerPhone = ""; item.F_Description = "";
                string itemJson = item.ToJson();
                itemJson = itemJson.Insert(1, "\"DepartmentId\":\"" + item.F_OrganizeId + "\",");
            ...
            {
                TreeGridEntity tree = new TreeGridEntity();
                bool hasChildren = organizedata.Count(t => t.F_ParentId == item.F_DepartmentId) == 0 ? false : true;
                tree.id = item.F_DepartmentId;
                if (item.F_ParentId == "0")
                {
                    tree.parentId = item.F_OrganizeId;
