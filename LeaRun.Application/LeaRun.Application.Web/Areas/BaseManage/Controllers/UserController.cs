...
namespace LeaRun.Application.Web.Areas.BaseManage.Controllers
{
    ...
    public class UserController : MvcControllerBase
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult SelectUserIndex()
        {
            return View();
        }
        #endregion
        ...
        public ActionResult GetTreeJson(string keyword)
        {
            ...
            foreach (UserEntity item in userdata)
            {
                ...
                tree.text = item.F_RealName;
                tree.value = item.F_Account;
                tree.parentId = item.F_DepartmentId;
                tree.title = item.F_RealName + "（" + item.F_Account + "）";
                tree.isexpand = true;
                tree.complete = true;
