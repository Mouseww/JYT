    ...
    public class OrganizeController : MvcControllerBase
    {
        ...
        public ActionResult GetTreeJson(string keyword)
        {
            ...
            foreach (OrganizeEntity item in data)
            {
                ...
                bool hasChildren = data.Count(t => t.F_ParentId == item.F_OrganizeId) == 0 ? false : true;
        ...
        public ActionResult GetTreeListJson(string condition, string keyword)
        {
            ...
            if (!string.IsNullOrEmpty(condition) && !string.IsNullOrEmpty(keyword))
            {
                ...
                {
                    case "FullName":    //公司名称
                        data = data.TreeWhere(t => t.F_FullName.Contains(keyword), "F_OrganizeId");
                        break;
                    case "EnCode":      //外文名称
                        data = data.TreeWhere(t => t.F_EnCode.Contains(keyword), "F_OrganizeId");
                        break;
                    case "ShortName":   //中文名称
                        data = data.TreeWhere(t => t.F_ShortName.Contains(keyword), "F_OrganizeId");
                        break;
                    case "Manager":     //负责人
                        data = data.TreeWhere(t => t.F_Manager.Contains(keyword), "F_OrganizeId");
                        break;
                    default:
                        break;
                }
