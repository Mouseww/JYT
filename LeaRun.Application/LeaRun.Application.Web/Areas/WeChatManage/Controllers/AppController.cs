    ...
    public class AppController : MvcControllerBase
    {
        ...
        public ActionResult RemoveForm(string keyValue)
        {
            ...
            if (Entity !=null)
            {
                ...
                DirFileHelper.DeleteFile(Entity.F_AppLogo);
            }
            return Success("删除成功。");
        }
        /// <summary>
        ...
        {
            var data = from items in menuJson.ToList<WeChatAppMenuEntity>() orderby items.F_SortCode select items;
            var treeList = new List<TreeEntity>();
            foreach (WeChatAppMenuEntity item in data)
            {
                TreeEntity tree = new TreeEntity();
                bool hasChildren = data.Count(t => t.F_ParentId == item.F_MenuId) == 0 ? false : true;
                tree.id = item.F_MenuId;
                tree.text = item.F_MenuName;
                tree.value = item.F_MenuId;
