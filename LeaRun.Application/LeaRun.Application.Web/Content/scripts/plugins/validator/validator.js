...
$.fn.Validform = function () {
                ...
                case "EmailOrNull":
                    {
                        if (!isEmailOrNull(value)) {
                            Validatemsg = errormsg + "E-mail格式或空！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "MobileOrPhone":
                    {
                        if (!isMobileOrPhone(value)) {
                            Validatemsg = errormsg + "电话格式或手机格式！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
                ...
                case "MobileOrPhoneOrNull":
                    {
                        if (!isMobileOrPhoneOrNull(value)) {
                            Validatemsg = errormsg + "电话格式或手机格式或空！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
    ...
    function isIntegerOrNull(obj) {
        ...
        if ( controlObj == null || controlObj == undefined || controlObj.length == 0) {
            return true;
        }
        reg = /^[-+]?\d+$/;
        if (!reg.test(obj)) {
...
function ValidationMessage(obj, Validatemsg) {
    try {
        removeMessage(obj);
        obj.focus();
        var $poptip_error = $('<div class="poptip"><span class="poptip-arrow poptip-arrow-top"><em>◆</em></span>' + Validatemsg + '</div>');//.css("ma", obj.parent().height() + 'px');//.css("left", obj.offset().left + 'px')
