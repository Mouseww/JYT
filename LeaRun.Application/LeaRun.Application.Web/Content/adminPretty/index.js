...
$.removeTab = function (type) {
    switch (type) {
        case "closeCurrent":
            top.$('.menuTabs .menuTab.active .tab_close').trigger('click');
            break;
        default:
            break;
    }
        ...
        loadMenu: function (isInit) {
            ...
            $.each(data, function (i) {
                ...
                if (row.F_ParentId == "0") {
                    ...
                    _itemHtml += '<a>';
                    _itemHtml += '<i class="' + row.F_Icon + '"></i><span>' + row.F_FullName + '</span>';
                    _itemHtml += '</a>';
                    ...
                    if (childNodes.length > 0) {
                        ...
                        $.each(childNodes, function (i) {
                            ...
                            if (subchildNodes.length > 0) {
                                _itemHtml += '<a class="menuTreeItem menuItem" href="#"><i class="' + subrow.F_Icon + ' firstIcon"></i>' + subrow.F_FullName + '';
                                ...
                                $.each(subchildNodes, function (i) {
                                    ...
                                    _itemHtml += '<li><a class="menuItem menuiframe" data-id="' + subchildNodesrow.F_ModuleId + '" href="' + subchildNodesrow.F_UrlAddress + '"><i class="' + subchildNodesrow.F_Icon + ' firstIcon"></i>' + subchildNodesrow.F_FullName + '</a></li>';
                                });
                                _itemHtml += '</ul></div>';
                            } else {
                                _itemHtml += '<a class="menuItem menuiframe" data-id="' + subrow.F_ModuleId + '" href="' + subrow.F_UrlAddress + '"><i class="' + subrow.F_Icon + ' firstIcon"></i>' + subrow.F_FullName + '</a>';
                            }
                            _itemHtml += '</li>';
                        });
                        _itemHtml += '</ul></div>';
