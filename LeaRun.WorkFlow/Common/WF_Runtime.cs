...
namespace LeaRun.WorkFlow
{
    ...
    public class WF_Runtime : IWF_Runtime
    {
        ...
        private string SqlBuider(dynamic schemeContentJson, string frmData, string keyValue)
        {
            try
            {
                if (schemeContentJson.Frm.F_isSystemTable.Value == 1)
                {
                    var strSql = new StringBuilder();
                    var frmDataParam = frmData.ToJObject();
                    string sqlname = schemeContentJson.Frm.F_FrmTableId.Value, sqlvalues = "'" + keyValue + "'";
                    ...
                    strSql.Append(string.Format("insert into " + schemeContentJson.Frm.F_FrmTable.Value + " ({0})values({1})", sqlname, sqlvalues));
